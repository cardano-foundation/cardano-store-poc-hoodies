...
cardano = BlockFrostChainContext(
    project_id=blockfrost_apikey, base_url=base_url)
...
main_address = Address(payment_part=payment_skey.to_verification_key().hash(
), staking_part=staking_skey.to_verification_key().hash(), network=cardano_network)
...
for asset in assets:
    ...
    certificate_authenticity = split_into_64chars(
        "This NFT represents a Certificate of Authenticity for the Cardano Foundation Proof of Concept Hoodie by linking the physical product to this digital asset on the Cardano blockchain leveraging an NFC chip embedded inside the hoodie.")
    ...
    payment_signing_key = placeholder_paymentkey.replace(
        'PLACEHOLDER', asset_private_key)
    payment_signing_key = PaymentSigningKey.from_json(payment_signing_key)

    result = cip8.sign(message=asset_name, signing_key=payment_signing_key,
                       network=cardano_network, attach_cose_key=True)

...
from config import SDMMAC_PARAM, ENC_FILE_DATA_PARAM, ENC_PICC_DATA_PARAM, SDM_MASTER_KEY
...
def _internal_sdm(with_tt=False, force_json=False):
    _, enc_picc_data_b, enc_file_data_b, sdmmac_b, asset_name = parse_parameters()
    ...
    try:
        ...
        payment_key = placeholder_paymentkey.replace(
            'PLACEHOLDER', file_data_utf8)
        ...
        payment_verification_key = PaymentVerificationKey.from_signing_key(
            payment_signing_key)
        payment_verification_key = str(payment_verification_key).split('"5820')[
            1].replace('"}', '')
        ...
        result = cip8.verify(signed_message=signed_message,
